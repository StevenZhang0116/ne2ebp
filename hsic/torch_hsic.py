...
if __name__ == "__main__":
    ...
    for epoch in range(HSIC_epochs):
        ...
        all_std.append(np.std(epoch_loss))
    
    plt.figure()
    plt.plot(range(len(hsic.remember)), hsic.remember)
    plt.title("Pre HSIC Step Loss")
    plt.xlabel("Steps")
    plt.ylabel("HSIC loss")
    plt.savefig("HSIC_step_loss.png")
    # plt.show()

    aa = [float(np.format_float_scientific(i, precision=2)) for i in all_loss]
    bb = [float(np.format_float_scientific(i, precision=2)) for i in all_std]
    print(aa)
    print(bb)
    plt.figure()
    plt.errorbar(range(HSIC_epochs), aa, yerr=bb, fmt='-o', ecolor='blue', capsize=5, label="hsic_loss")
    ...
    plt.title("Pre HSIC Epoch Loss")
    plt.xlabel("Epochs")
    plt.ylabel("HSIC loss")
    plt.legend()
    plt.savefig("HSIC_epoch_loss.png")
    ...
    # plt.figure()
    # plt.plot(range(HSIC_epochs), all_loss)
    # plt.title("Post HSIC Epoch Loss")
    # plt.xlabel("Epochs")
    # plt.ylabel("HSIC loss")
    # plt.savefig("HSIC_epoch_loss_pos.png")
